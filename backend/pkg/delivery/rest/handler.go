...
import (
	...
	"github.com/codenation-dev/squad-4-aceleradev-fs-florianopolis/backend/pkg/api"
...
func Handler(
	...
	router.HandleFunc("/customer", s.Middleware(s.getCustomerByName)).Methods("GET")
	...
	// Funcionario
	router.HandleFunc("/funcionario", s.Middleware(api.FuncionarioList)).Methods("GET")
	router.HandleFunc("/funcionario/detail", s.Middleware(api.FuncionarioGet)).Methods("GET")

	//Cliente
	router.HandleFunc("/cliente", s.Middleware(api.ClienteList)).Methods("GET")
