...
// ReadCustomer read a customers from the DB, given the id
func (s *Storage) ReadCustomer(id int) (*Customer, error) {
	c := new(Customer)
	query := "SELECT * FROM customers WHERE id=$1"
	err := s.db.QueryRow(query, id).Scan(&c)
	if err != nil {
		return nil, err
	}
	return c, nil
...
func (s *Storage) GetAllCustomers() ([]*reading.Customer, error) {
	...
	for rows.Next() {
		c := new(reading.Customer)
		err := rows.Scan(&c) // Se der erro aqui, tentar retirar o &
		if err != nil {
			return nil, err
		}
